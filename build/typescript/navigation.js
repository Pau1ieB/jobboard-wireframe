...
let profile = { type: { value: "main" } };
let jobProfile = {};
//handles all hash chnages
export const ChangePage = (action, args) => __awaiter(void 0, void 0, void 0, function* () {
    //if page is refreshed will restart db
    if (!CheckDB())
        yield OpenDataBase(dbStores);
    //load profile + pictures from database, if required
    if (args[1] === "employers" || args[1] === "applicants") {
        let response = yield ReadData(args[1], "id", 1, []);
        ...
        response = yield ReadData('images', "name", profile.profile.value, []);
        console.log(response);
        profile.profileImage = response.data[0][0].src;
        if (args[1] === 'employers') {
            response = yield ReadData('images', "name", profile.logo.value, []);
            console.log(response);
    ...
    if (args[0] === "display") {
        ...
        if (action === "MainPage" || action === 'ApplicantDashboard' || action === 'EmployerDashboard') {
            ...
            const response = (action === 'EmployerDashboard') ? yield ReadDataByRangeAndKey("jobs", "keyCompany", "Digital Native", page.lower, page.upper) : yield ReadDataByRange("jobs", page.lower, page.upper);
            console.log(response);
            ...
            const response = yield ReadData('jobs', 'id', parseInt(args[1]), []);
            ...
            if (jobProfile.applicants.value.includes(profile.id))
                profileButtons.push({ text: 'Remove Application', hash: '#ApplyForJob?false' });
            else
                profileButtons.push({ text: 'Apply', hash: '#ApplyForJob?true' });
            profileButtons.push({ text: 'Back', hash: '#ApplyForJob?no' });
        }
        else if (action === 'ViewApplicant') {
            let response = yield ReadData('jobs', 'id', parseInt(args[1]), []);
            if (response.data[0].applicants.value.length == 0) {
                alert("There are no applicants");
                return;
            }
            response = yield ReadData('applicants', 'id', response.data[0].applicants.value[0], []);
            console.log(response);
        ...
        if (navString.length > 0) {
            const elem = document.querySelector('[data-pagenav]');
        ...
        if (action === 'NewJobProfile' || action === 'EditJobProfile') {
            ...
            const comboList = [...document.querySelectorAll('input[type="checkbox"]')];
            comboList.forEach((combo) => combo.addEventListener('change', ChangeJobLocation));
        }
    }
        ...
        if (CheckValuesForChange(profile, inputValues.values)) {
            ...
            const response = yield PutData(profile.type.value, cloneProfile);
            console.log(response);
        }
        ...
        if (CheckValuesForChange(profile, inputValues.values)) {
            if (!confirm("Do you want to abandon your changes?")) {
                const elem = document.querySelector("a:nth-child(2)");
        ...
        if (inputs.emptyCount > 0) {
            if (!confirm('Not all fields are filled in. Do you want to create this job now?')) {
                const elem = document.querySelector('a:nth-child(1)');
        ...
        const response = yield WriteSingleData('jobs', jobProfile);
        console.log(response);
        jobProfile = {};
        ...
        if (!confirm('Are you sure you want to delete this Job?')) {
            const elem = document.getElementById(`delete-${args[0]}`);
            ResetAnchorElementHref(elem, `#DeleteJob?${args[0]}`, args[1]);
            return;
        }
        const response = yield DeleteData('jobs', parseInt(args[0]));
        console.log(response);
        location.hash = '#EmployerDashboard?display';
    }
        ...
        if (CheckValuesForChange(jobProfile, inputs.values)) {
            ChangesToProfile(jobProfile, inputs.values);
            RemoveJobLists(jobProfile);
            const response = yield PutData('jobs', jobProfile);
            console.log(response);
        }
        jobProfile = {};
        location.hash = '#EmployerDashboard?display';
        ...
        if (args[0] === 'true')
            ...
            jobProfile.applicants.value = jobProfile.applicants.value.filter((id) => id != profile.id);
        if (args[0] !== 'no') {
            const response = yield PutData('jobs', jobProfile);
            console.log(response);
        }
        ...
        if (args[0] === 'Profile' || args[0] === 'Logo') {
            const input = document.getElementById('load-image-data');
            AssignCallbackFileString(args[0]);
            AssignLoadFileCallback(HandleImageInput);
            input.click();
        }
        const elem = document.getElementById('fileinput-' + args[0]);
        ResetAnchorElementHref(elem, '#FileInput?' + args[0], args[1]);
    }
    else if (action === 'RemoveFileInput') {
        const elem = document.getElementById('removefileinput-' + args[0]);
    ...
    //functions for the job pagination system
        ...
        if (args[0] === 'down') {
            if (JobPageDown()) {
                UpdateJobsInput();
                const page = GetJobPage();
                const response = (profile.type.value === 'employers') ? yield ReadDataByRangeAndKey("jobs", "keyCompany", "Digital Native", page.lower, page.upper) : yield ReadDataByRange('jobs', page.lower, page.upper);
                const tableString = GetJobsTable(response.data, profile);
                const section = document.querySelector('section');
                section.innerHTML = tableString;
            }
            const elem = document.querySelector('[data-job="down"]');
            ...
            const elem = document.querySelector('[data-job="up"]');
            ResetAnchorElementHref(elem, '#JobSearch?up', args[1]);
        }
        else if (args[0] === 'input') {
            const page = GetJobPage();
            const input = document.querySelector('[data-pagenav]');
            ...
            const response = (profile.type.value === 'employers') ? yield ReadDataByRangeAndKey("jobs", "keyCompany", "Digital Native", lower, upper) : yield ReadDataByRange('jobs', lower, upper);
            if (response.ok == 1) {
                input.value = page.current;
                return;
            }
...
const ChangeJobLocation = (event) => {
    if (event.currentTarget.checked)
        ...
        jobProfile.location.value = jobProfile.location.value.split('/').filter((obj) => obj !== event.currentTarget.labels[0].textContent).join('/');
    const elem = document.getElementById('combo-container');
    if (elem != null) {
        elem.childNodes[0].textContent = jobProfile.location.value;
    }
};
const HandleImageInput = (data, fileName, fileString) => __awaiter(void 0, void 0, void 0, function* () {
...
const ChangeImage = (data, fileName, className, imageProfile, labelName) => __awaiter(void 0, void 0, void 0, function* () {
    let response = null;
    ...
    if (imageProfile.value !== 'anon-person.png') {
        response = yield ReadData('images', 'name', imageProfile.value, []);
        console.log(response);
        response = yield DeleteData('images', response.data[0][0].id);
        console.log(response);
    }
    response = yield WriteSingleData('images', { name: fileName, src: data });
    console.log(response);
    ...
    response = yield PutData(profile.type.value, cloneProfile);
    console.log(response);
    if (className.length == 0)
        profile.logoImage = data;
    else {
